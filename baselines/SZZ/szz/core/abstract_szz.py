...
class AbstractSZZ(ABC):
    ...
    def __init__(
        ...
        if not use_temp_dir:
            self.__temp_dir = os.path.join(os.getcwd(), "tmp_szz")
        ...
        for entry in self.repository.blame_incremental(
            **kwargs, rev=rev, L=mod_line_ranges, file=file_path
            ...
            for line_num in entry.orig_linenos:  #
                ...
                line_str = lines[line_num - 1].strip()
    ...
    def _parse_line_ranges(self, modified_lines: List) -> List[str]:
        ...
        modified_lines = sorted(set(modified_lines))

        start = modified_lines[0]
        end = modified_lines[0]
